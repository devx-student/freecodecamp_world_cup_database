...
echo "$($PSQL "SELECT SUM(winner_goals) AS total_winner_goals FROM games")"
...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals) + AVG(opponent_goals),16) AS average_total_goals FROM games")"

echo -e "\nMost goals scored in a single game by one team:"
echo "$($PSQL "SELECT MAX(winner_goals) AS max_winner_goals FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(game_id) AS more_than_two_winner_goals FROM games WHERE winner_goals > 2")"
...
echo "$($PSQL "SELECT DISTINCT name FROM games LEFT JOIN teams on games.winner_id = teams.team_id WHERE name LIKE 'Co%' ORDER BY name")"
